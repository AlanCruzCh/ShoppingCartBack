...
public class ShoppingCartCC {
    ...
    private ResponseEntity<?> saveArticule(@RequestBody ArticuloDTO json){
        try {
            ...
            return ResponseEntity.status(HttpStatus.CREATED).body(
                new ResponseTypeDtoMessaje("Se ha creado con exito el el arituvlo con el id "+ newArticule.getIdArticulo()));
    ...
    @DeleteMapping("/limpia/carrito")
    private ResponseEntity<?> deleteCarrito(){
        try {
            return ResponseEntity.status(HttpStatus.OK).body(
                new ResponseTypeDtoMessaje(shoppingCartCN.deleteShoppingCart())
            );
        } catch (DataNotFound e) {
            return ResponseEntity.status(HttpStatus.METHOD_NOT_ALLOWED).body(e.getMessage());
        } catch (DatabaseAccessException e) {
    ...
    @DeleteMapping("/limpia/carrito/articulo/{idCarrito}")
    private ResponseEntity<?> deleteCarritoArticulo(@PathVariable Integer idCarrito){
        try {
            return ResponseEntity.status(HttpStatus.OK).body(
                new ResponseTypeDtoMessaje(shoppingCartCN.deleteArticuloCarrito(idCarrito))
            );
        } catch (DataNotFound e) {
            return ResponseEntity.status(HttpStatus.METHOD_NOT_ALLOWED).body(e.getMessage());
        } catch (DatabaseAccessException e) {
    ...
    @PutMapping("/actualiza/cantidad/carrito/articulo")
    private ResponseEntity<?> updateCarritoArticulo(@RequestBody CarritoArticuloUpdateDto dataJson){
        try {
            return ResponseEntity.status(HttpStatus.OK).body(
                new ResponseTypeDtoMessaje(shoppingCartCN.updateArticuloCarrito(dataJson))
                );
        } catch (DataNotFound e) {
            return ResponseEntity.status(HttpStatus.METHOD_NOT_ALLOWED).body(e.getMessage());
        } catch (DatabaseAccessException e) {
